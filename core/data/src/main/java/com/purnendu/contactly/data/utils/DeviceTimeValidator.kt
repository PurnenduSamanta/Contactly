...
class DeviceTimeValidationRepository(
    private val context: Context
) : TimeValidationRepository {
    override suspend fun validateDeviceTime(): TimeValidationResult {
        ...
        val networkCalendar = Calendar.getInstance(TimeZone.getTimeZone(response.timeZone)).apply {
            ...
            set(Calendar.MONTH, response.month - 1)
        ...
        if (driftMinutes > MAX_ALLOWED_DRIFT_MINUTES) {
            return TimeValidationResult(
                isValid = false,
